...
local technology_name = "titanium-barrels"
-- The base empty barrel item
local empty_barrel_name = "titanium-barrel"
-----------------------------------------------------------------------------------------------------------------------
local barrel_empty_icon = "__barreling_machines__/graphics/titanium-barreling/barrel-empty.png"
local barrel_empty_side_mask = "__barreling_machines__/graphics/titanium-barreling/barrel-empty-side-mask.png"
local barrel_empty_top_mask = "__barreling_machines__/graphics/titanium-barreling/barrel-empty-top-mask.png"

local barrel_side_mask = "__barreling_machines__/graphics/titanium-barreling/barrel-side-mask.png"
local barrel_fill_icon = "__barreling_machines__/graphics/titanium-barreling/empty-barrel.png"
local barrel_fill_top_mask = "__barreling_machines__/graphics/titanium-barreling/barrel-fill-top-mask.png"
-----------------------------------------------------------------------------------------------------------------------
...
for fluid_name, value in pairs(titanium_barrel_fluids) do
	if data.raw.fluid[fluid_name] then
		data.raw.fluid[fluid_name].auto_titanium_barrel = value
	end
end
...
local function create_barrel_item(name, fluid, empty_barrel_item)
	local result = {
		type = "item",
		name = name,
		localised_name = { "item-name.filled-titanium-barrel", fluid.localised_name or { "fluid-name." .. fluid.name } },
	...
	if name == "fluoroketone-cold-titanium-barrel" then
...
local function create_fill_barrel_recipe(item, fluid)
	...
	local recipe = {
		type = "recipe",
		name = recipe_name,
		localised_name = { "recipe-name.fill-titanium-barrel", fluid.localised_name or { "fluid-name." .. fluid.name } },
		categories = { "titanium-barreling" },
		...
		--factoriopedia_alternative = "titanium-barrel",
...
local function process_fluid(fluid, technology, empty_barrel_item)
	if (fluid.auto_barrel == false) and (fluid.auto_titanium_barrel ~= true) then
		return
	end
	...
	local barrel_name = fluid.name .. "-titanium-barrel"
